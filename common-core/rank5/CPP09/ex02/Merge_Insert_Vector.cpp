...
MergeInsertVector::MergeInsertVector() {};
MergeInsertVector::MergeInsertVector(const MergeInsertVector& other): main(other.main), pend(other.pend) {}
MergeInsertVector& MergeInsertVector::operator=(const MergeInsertVector& other) {
	if (this != &other) {
		main = other.main;
		pend = other.pend;
	}
...
static bool lstContain(std::vector<int> &main, int nb) {
...
void	MergeInsertVector::fillMain(std::string str) {
	...
	while (iss >> token) {
    	...
    	ss >> nb;
		if (nb < 0) {
			throw std::runtime_error(std::string("Negative number : ") + token);
		}
...
static int	pairDivision(std::vector<int> &main, int lvl) {
	int e_size = 1 << lvl;
...
static void addVectorToVector(std::vector<int> &src, std::vector<int> &dst, int lvl, int i) {
	int e_size = 1 << lvl;
	for (int index = i ; index < i + e_size ; ++index)
		dst.push_back(src[index]);
}

static void	pendSplitter(std::vector<int> &main, std::vector<int> &pend, std::vector<int> &rest, int lvl) {
	int	e_size = 1 << lvl;
	std::vector<int> cpy = main;
	main.clear();
	pend.clear();
	rest.clear();
...
static std::vector<int> getJacobSequence(int limit) {
...
static void	mergeRest(std::vector<int> &main, std::vector<int> &rest) {
...
static void addDequetoVectorSort(std::vector<int> &dst, std::vector<int> &dstRef, std::vector<int> &src, int id, int lvl) {
	int e_size = 1 << lvl;
	int	dstRefMaxVal;
...
static void pendMerger(std::vector<int> &main, std::vector<int> &pend, std::vector<int> &rest, int lvl) {
	if (pend.empty())
		return (mergeRest(main, rest));
	int e_size = 1 << lvl;
	std::vector<int> jac = getJacobSequence(pend.size() / e_size);

	std::vector<int> mainRef = main;
...
void	MergeInsertVector::checkSort() {
	for (size_t i = 1 ; i < main.size() ; i++) {
		if (main[i] < main[i - 1]) {
			std::cout << "Sorting vector BAD !" << std::endl;
			return ;
		}
	}
	for (size_t i = 0 ; i < main.size() ; i++) {
	...
	// std::cout << "Sorting vector GOOD !" << std::endl;
}

std::vector<int>&	MergeInsertVector::getMain() {
	return (main);
}